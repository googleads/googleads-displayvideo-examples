...
class AppendAudienceAssignedTargetingOption extends BaseExample
{
    ...
    public function run()
    {
        ...
        $additionalGoogleAudiences = explode(",", $values['additional_google_audiences']);
        ...
        // Call the API, retrieving the existing audience targeting for the line
        // item.
        try {
            $response = $this
                ->service
        ...
        // Create a bulk edit request. It will delete any existing targeting option and create the
        // updated audience targeting option.
        $bulkEditRequest =
           new Google_Service_DisplayVideo_BulkEditLineItemAssignedTargetingOptionsRequest();

        // Iterate over existing targeting, find audience targeting option, retrieve the details and
        // set it to be deleted in the bulk edit.
        ...
        foreach ($response->getAssignedTargetingOptions() as $option) {
            if ($option->getAssignedTargetingOptionId() == AppendAudienceAssignedTargetingOption::AUDIENCE_TARGETING_OPTION_ID) {
                $updatedAudienceGroupDetails =
        ...
        // Print information on the new audience assigned targeting option.
        $this->printAudienceAssignedTargetingOptions(
    ...
    /**
     * Prints the audience details of the given assigned targeting options.
     * @param array $options created assigned targeting options to be printed.
     */
    protected static function printAudienceAssignedTargetingOptions(array $options)
    {
        if (empty($options)) {
            print '<p>No assigned targeting options created</p>';
        } else {
            foreach ($options as $option) {
                print '<p>An assigned targeting option positively targeting '
                    .'the following Google audience IDs was created:</p>';
                print '<ul>';
                $googleAudiences =
                    $option
                        ...
                        ->getSettings();
                foreach ($googleAudiences as $googleAudienceSetting) {
                    printf(
                        '<li>%s</li>',
                        $googleAudienceSetting->getGoogleAudienceId()
                    );
                }
                print '</ul>';
            }
