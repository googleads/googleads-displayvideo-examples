...
class EditUserAccess extends BaseExample
{
    ...
    protected function getInputParameters(): array
    {
        return array(
            ...
            array(
                ...
                'display' => 'Partner IDs to remove access to (comma-separated)',
                'required' => false
            ),
            array(
                'name' => 'remove_advertiser_ids',
                'display' => 'Advertiser IDs to remove access to (comma-separated)',
                'required' => false
            ),
            array(
                'name' => 'add_partner_roles',
                'display' => 'List of partner ID and user role pairings to add to the user. '
                    . 'Pairings split by semicolons, entries comma-separated '
                    . '(Ex: 123;ADMIN,456;ADMIN)',
                'required' => false
            ),
            array(
                'name' => 'add_advertiser_roles',
                'display' => 'List of advertiser ID and user role pairings to add to the user. '
                    . 'Pairings split by semicolons, entries comma-separated '
                    . '(Ex: 123;STANDARD,456;READ_ONLY)',
                'required' => false
            )
        );
    }
    ...
    public function run()
    {
        ...
        if (!empty($values['remove_advertiser_ids'])) {
            $removeAdvertiserIds = explode(",", $values['remove_advertiser_ids']);
        }
        if (!empty($values['add_partner_roles'])) {
            $addPartnerRoles = explode(",", $values['add_partner_roles']);
        }
        if (!empty($values['add_advertiser_roles'])) {
            $addAdvertiserRoles = explode(",", $values['add_advertiser_roles']);
        }

        // Create list to store the roles to add to user.
        $createdAssignedUserRoles = array();

        // Create assigned user roles from given arguments and add to list of user roles to add to
        // the user.
        ...
        // Call the API, editing the assigned user roles for the identified user.
        try {
            $response = $this
                ->service
                ->users
     ...
     * @param array $assignedUserRoles created assigned user roles to be printed.
     */
    protected function printCreatedAssignedUserRoles(string $userId, array $assignedUserRoles)
    {
