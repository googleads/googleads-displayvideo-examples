...
/**
 * This example retrieves accessible Display & Video 360 users based on given filter
 * values.
 *
...
class RetrieveUsers extends BaseExample
{
    ...
    public function run()
    {
        ...
        if (!empty($userRoleEntityType)) {
            $filters[] = 'assignedUserRole.entityType="' . $userRoleEntityType . '"';
        }
        if (!empty($partnerId)) {
        ...
        if (!empty($parentPartnerId)) {
            $filters[] = 'assignedUserRole.parentPartnerId="' . $parentPartnerId .'"';
        }
        ...
        do {
            ...
            // Call the API, retrieving a page of accessible users matching the given filter
            // values.
            try {
                $response = $this
                    ->service
    ...
    protected function printUsers(array $users)
    {
        if (!empty($users)) {
            ...
            foreach ($users as $user) {
                printf('<li>User %s found:<ul><li>Email address: %s</li><li>Display name: '
                    . '%s</li><li>Assigned User Roles:<ul>',
                    $user['name'],
                    $user['email'],
                    $user['displayName']
                );
                ...
                foreach ($user['assignedUserRoles'] as $userRole) {
                    if (isset($userRole['partnerId'])) {
                        ...
                        printf('<li>Advertiser ID: %s, User Role: %s</li>',
