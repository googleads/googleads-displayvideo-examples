...
The line item will inherit settings, including targeting, from the insertion
order. If generating a Mobile App Install line item, an app ID must be provided.
"""
...
def main(service, flags):
    """Generates a default line item under an insertion order.
    ...
    if flags.line_item_type in [
        ...
        if not flags.app_id:
            generate_default_line_item_request["mobileApp"] = {
                "appId": flags.app_id
            }
        else:
            print(
                "Error: No app ID given for Mobile App Install line item. "
                "Exiting."
            )
            sys.exit(1)
    ...
    response = (
        ...
        .generateDefault(
            ...
            body=generate_default_line_item_request,
        )
        .execute()
    )
...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        "--app_id",
        help="The app ID of the mobile app promoted by the line item. Required "
        "and only valid if line item type is either "
